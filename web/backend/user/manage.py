...
from user.config import CLIENT_ID, CLIENT_SECRET
from authlib.integrations.starlette_client import OAuth, OAuthError
from user.verify_email import send_verification_email, generate_verification_code
...
oauth.register(
    ...
    client_kwargs={
        ...
        'redirect_url': 'http://localhost:8000/user/auth'
    }
)
@route.post("/signup")
...
async def login(request: Request, form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    try:
        user = db.fetch_user({"email":form_data.username, "password":form_data.password})
        if user:
            ...
            # return {"message": "Login success!", "token": access_token, "user": user["fullname"]}
        else:
...
async def auth(request: Request):
    ...
    try:
        ...
        if user:
            ...
            temp_pwd = generate_verification_code()
            db.create_user(fullname, email, temp_pwd)
            request.session['user'] = {'name': fullname, 'email': email}
    except Exception as e:
        return {'error': str(e)}
    return RedirectResponse('/home')
...
def forgot_password(request: Request, email: str = Form(...)):
    try:
        ...
        if user:
            ...
            db.update_user(user['email'], {"verification_code": code})
            request.session['user_email'] = email
            message = send_verification_email(email, user['fullname'], code)
...
def reset_password(request: Request, new_pwd: str = Form(...), confirm_new_pwd: str = Form(...)):
    try:
        email = request.session.get('user_email')
        user = db.fetch_user({"email":email})
        if user:
            if new_pwd == confirm_new_pwd:
                db.update_user(user['email'], {"password": new_pwd})
                content="""
            <html>
        <head>
        ...
        <body>
            <p>
            <b style='color:green;'>
                <img width='20px' height='20px' src='https://cdn-icons-png.flaticon.com/256/6784/6784655.png'>
                  Password changed, try login now!</b>
            </p>
        </body>
        </html>
        """
